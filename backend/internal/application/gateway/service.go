...
attemptLoop:
	...
	handleResponse:
		...
		afterTeamRateLimit:
			...
			if !lastFailure.AccountScoped && lastFailure.ModelCapacity {
				if attemptPolicy.allows(attempt+1) && !waitForModelCapacityRetry(ctx, attempt, retryAfter) {
					lastErr = ctx.Err()
					break
				}
			} else if shouldStopForNonAccountFingerprint(failureFingerprints, lastFailure) {
				break
			}
			continue
		}
